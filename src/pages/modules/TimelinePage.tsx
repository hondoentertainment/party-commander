...
import type { ItemStatus } from '../../state/types'
...
export function TimelinePage() {
  ...
  const updateTaskStatus = (id: string, status: ItemStatus) => {
    if (isPartyScope) {
      const nextTasks = state.timeline.tasks.map((t) =>
        t.id === id ? { ...t, status } : t,
      )
      dispatch({ type: 'update_timeline', payload: { tasks: nextTasks } })
    } else {
      const ev = state.events.items.find((e) => e.id === selectedEventId)
      if (!ev) return
      const nextTasks = (ev.tasks ?? []).map((t) => (t.id === id ? { ...t, status } : t))
      dispatch({
        type: 'update_events',
        payload: {
          items: state.events.items.map((e) =>
            ...
            {currentTasks.map((task) => {
              ...
              return (
                <div
                  key={task.id}
                  className={cn(
                    'flex flex-wrap items-center justify-between gap-2 rounded-xl bg-white/5 px-4 py-3 text-sm',
                    task.status === 'done' && 'opacity-75',
                  )}
                >
                  <div className="min-w-0 flex-1">
                    <p className={cn('font-semibold text-white', task.status === 'done' && 'line-through')}>
                      {task.title}
                    </p>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <Select
                      value={task.status}
                      onChange={(e) => updateTaskStatus(task.id, e.target.value as ItemStatus)}
                      className="w-36"
